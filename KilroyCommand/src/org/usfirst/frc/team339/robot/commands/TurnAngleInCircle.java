 ...
 * 
 * @param angle
 *            to turn about the center
 * @param radius
 *            of the circle
 * @param baseMotorRatio
 *            to be the speed of the faster wheel
 * @param turnRight
 *            false if turning left
 */
public TurnAngleInCircle (double angle, double radius,
        double baseMotorRatio, boolean turnRight)
{
    ...
    this.outsideFinalDistance = Math.toRadians(this.angle)
            * (this.radius + (TurnAngleInCircle.wheelDiameter / 2));
    this.insideFinalDistance = Math.toRadians(this.angle)
            * (this.radius - (TurnAngleInCircle.wheelDiameter / 2));

    this.insideBaseMotorRatio = this.outsideBaseMotorRatio
            * (this.insideFinalDistance / this.outsideFinalDistance);
...
protected void execute ()
{
    ...
    if (this.turningRight == false)
        ...
        if (this.angle > 0)
            {
            if (Subsystems.encoders.getLeftDistance() > (leftMotorRatio
                    / rightMotorRatio)
                    * Subsystems.encoders.getRightDistance())
                {
                leftMotorRatio *= correctionFactor;
                }
            else if (Subsystems.encoders
                    .getLeftDistance() < (leftMotorRatio
                            / rightMotorRatio)
                            * Subsystems.encoders.getRightDistance())
                {
        ...
        if (this.angle > 0)
            {
            if (Subsystems.encoders
                    .getRightDistance() > (rightMotorRatio
                            / leftMotorRatio)
                            * Subsystems.encoders.getLeftDistance())
                {
                rightMotorRatio *= correctionFactor;
                }
            else if (Subsystems.encoders
...
protected boolean isFinished ()
{
    if (turningRight == false)
        ...
        if (angle > 0)
            {
            if (2 * (Subsystems.encoders.getRightDistance()
                    - Subsystems.encoders.getLeftDistance())
                    / TurnAngleInCircle.wheelDiameter >= angle)
                {
                return true;
                }
            }
        ...
        if (angle > 0)
            {
            if (2 * (Subsystems.encoders.getLeftDistance()
                    - Subsystems.encoders.getRightDistance())
                    / TurnAngleInCircle.wheelDiameter >= angle)
                {
                return true;
