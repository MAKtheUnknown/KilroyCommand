...
public class DriveToGoalByCamera extends Command {
    ...
    protected void execute() 
    {
    	...
    	if(Subsystems.goalVision.getProportionalGoalY() < Y_OFFSET - Y_DEADBAND)
    	{
    		leftMotor = this.DEFAULT_MOTOR_RATIO;
    		rightMotor = this.DEFAULT_MOTOR_RATIO;
    	}
    	else if(Subsystems.goalVision.getProportionalGoalY() > Y_OFFSET + Y_DEADBAND)
    	{
    		leftMotor = -this.DEFAULT_MOTOR_RATIO;
    		rightMotor = -this.DEFAULT_MOTOR_RATIO;
    	}
    	
    	if(Subsystems.goalVision.getProportionalGoalX() < X_OFFSET - X_DEADBAND)
    	{
    ...
    {
    	if((Subsystems.goalVision.getProportionalGoalX() >= X_OFFSET - X_DEADBAND
    			&& Subsystems.goalVision.getProportionalGoalX() <= X_OFFSET + X_DEADBAND)
    			&&(Subsystems.goalVision.getProportionalGoalY() >= Y_OFFSET - Y_DEADBAND
    			&& Subsystems.goalVision.getProportionalGoalY() <= Y_OFFSET + Y_DEADBAND))
    	{
    		return true;
    	}
